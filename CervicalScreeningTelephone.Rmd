...
Over the next three months, twelve of those patients (4.4%) had a recorded cervical screening.

In the next three months after that, an additional six patients (2.2%) had a recorded cervical screening.
...
Previous proportion of under-screened patients who were screened in the three months after March 2018 was 4.4%. With a populaton of 290 patients, split between two groups, power of 0.8 and significance level of 0.05:
...
```{r}
p1 <- 0.044        # our previously observed proportion of women
                   # who had cervical screening (CST) over three months
p2 <- 0.1383313    # our 'guess' of what the 'treatment' group who had CST over
...
abs(qnorm(0.2)+qnorm(0.025))*sqrt(1/((0.5)*(1-0.5)))*sqrt((p_avg)*(1-p_avg)/290)
```

Previous proportion of under-screened patients who were screened in the six months after March 2018 was 6.6%. With a populaton of 290 patients, split between two groups, power of 0.8 and significance level of 0.05:
...
sub-groups, and export to Excel file for use by
surveyors.
...
treatment1_details <- treatment1 %>%
  ...
  select(-c("PENSIONNO", "INTERNALID")) # InternalID is not needed any more!
...
for (i in levels(treatment1_details$AgeGroup5)) {
  ...
  sheetname1 <- paste("Treat Group 1 - ", i)
  ...
  sheetname2 <- paste("Treat Group 2 - ", i)
  addWorksheet(wb_treat, sheetname2)
  writeData(wb_treat, sheetname2, subsection2)
}

saveWorkbook(wb_treat, file = "20190301CSTTreatmentGroups.xlsx", overwrite = TRUE)
...
for (i in levels(control1_details$AgeGroup5)) {
  ...
  sheetname1 <- paste("Control Group 1 - ", i)
  ...
  sheetname2 <- paste("Control Group 2 - ", i)
  addWorksheet(wb_control, sheetname2)
  writeData(wb_control, sheetname2, subsection2)
}

saveWorkbook(wb_control, file = "20190301CSTControlGroups.xlsx", overwrite = TRUE)
